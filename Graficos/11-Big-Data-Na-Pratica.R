setwd("C:\\Cursos\\Cuso_DSA\\praticando_dia_a_dia\\web_app")

###### Script de Carga e limpeza ######
...
#carregando os dados do WordBank
...
#carregando o IMBD
dados_IMDB <- read_tsv("imdb/data.tsv")
View(dados_IMDB)

#carregando dados top10 shows da netflix
dados_top10 <- read_excel("datasets_originais/top_10_shows_netflix.xlsx")
View(dados_top10)

#carregando dados de assinantes de 2021/julho
dados_sub <- read.csv("datasets_originais/assinantes_netflix_jul_2021.csv")
View(dados_sub)

#carregando codigo iso dos paises
countrycode <- read.csv("datasets_originais/wikipedia-iso-country-codes.csv")
View(country_code)

###### Limpeza e preparação do Priemiro Dataset Combinado ######

#Cria a coluna com a diferença de dados para o gráfico de barras(plano basico)
dados_netflix$standard_diff = (dados_netflix$Cost.Per.Month...Standard....- dados_netflix$Cost.Per.Month...Basic....)

#Cria a coluna com a diferença de dados para o gráfico de barras(plano standard)
dados_netflix$standard_premiun_diff = (dados_netflix$Cost.Per.Month...Premium.... - dados_netflix$Cost.Per.Month...Standard....)


#combina os dados anteriores com os dados do pib
...
#limpeza do dataset desigualdade salarial
dados_salario <- dados_salario[, c(1:3)]
dados_salario_ano <- dados_salario %>% group_by(country)%>% summarise(max = max(year, na.rm = TRUE)) #nessa linha o valor ausente está sendo removido
...
#fazendo o merge do countrycode para chorpleth map
countrycode <- countrycode[,c(1, 3)]
complete <- merge(complete, countrycode, by.x=c("Country"), by.y=c("English.short.name.lower.case"))
View(complete)

#grava o DataFrame em Disco.
write.csv(complete, "datasets_limpos/dataset1.csv",row.names= F)

#limpando e filtrando o dataset MDB
genero <- dados_IMDB[, -c(1, 4:8)]
View(genero)
#Renomeando a coluna para fazer a junção.
names(genero)[names(genero) == 'primaryTitle']<- 'show_title'

#associa o genero com os 10 top shows
topgenero <- merge(dados_top10, genero, by = "show_title")
View(topgenero)

#limpa o dataframe anterior para manter uma unica entrada para cada top10
...
#salva em disco
write.csv(generocount, "datasets_limpos/dataset2.csv", row.names = F)


#renomear o dataset anterior
...
write.csv(sunburst, "datasets_limpos/dataset3.csv", row.names = F)

##### Limpeza e Preparação do Quarto Dataset Combinado ##### 

#trabalhar com top 10 para evitar problemas de performance nos gráficos
...
#salva em disco o dataset
write.csv(top10sunburst, "datasets_limpos/dataset4.csv", row.names = F)
